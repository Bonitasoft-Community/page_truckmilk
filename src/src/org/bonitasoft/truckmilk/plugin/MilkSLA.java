...
public class MilkSLA extends MilkPlugIn {
  ...
  private static PlugInParameter cstParamProcessName = PlugInParameter.createInstance("ProcessName", TypeParameter.PROCESSNAME, "", "Process name is mandatory. You can specify the process AND the version, or only the process name: all versions of this process is then checked");

  private static PlugInParameter cstParamRuleSLA = PlugInParameter.createInstanceArrayMap("RuleSLA",
      Arrays.asList(
  ...
  private static PlugInParameter cstParamEmailSubject = PlugInParameter.createInstance("EmailSubject", TypeParameter.STRING, "The task {{taskName}} in case {{caseId}} need your attention", "Subject of the mail. See the content to the list of Place holder allowed");
  private static PlugInParameter cstParamEmailContent = PlugInParameter.createInstance("EmailContent", TypeParameter.TEXT, "A task is close to its due date. Access the {{taskUrl}}",
  ...
  private static PlugInParameter cstParamEmailBonitaServer = PlugInParameter.createInstance("EmailBonitaServer", TypeParameter.STRING, "http://localhost:8080", "HTTP address to join the caseId, if you place the <i>taskURL</i> in the content on the mail, to access the Bonita server");
  private static PlugInParameter cstParamProcessVariableRegistration = PlugInParameter.createInstance("ProcessVariableRegistration", TypeParameter.STRING, null, "To avoid to execute the same rule, give access to a Process Variable HASHMAP, then all executions are registered on this data");
  private static PlugInParameter cstParamDelaiRegistrationInMn = PlugInParameter.createInstance("DelaiRegistrationInMinute", TypeParameter.LONG, null,
  ...
  private static PlugInParameter cstParamMaximumTask = PlugInParameter.createInstance("MaximumTask", TypeParameter.LONG, 10000L, "In order to protect the server, this is the maxim task the rule will check in the process");

  public static PlugInParameter cstParamAnalyseCaseId = PlugInParameter.createInstanceButton("AnalyseCaseId", Arrays.asList("CaseId"), Arrays.asList(""), "Give a caseId and an explanation will be return to describe what was done for this caseId", "Click on the button to start the analysis");
  ...
  public PlugInDescription getDescription() {
    ...
    plugInDescription.displayName = "SLA";
  ...
  public PlugTourOutput executeSLA(PlugTourInput input, Long analysisCaseId, APIAccessor apiAccessor)
  {
  ...
  private boolean isRuleActive(HumanTaskInstance humanTask, Rule rule, PlugTourInput input, ProcessAPI processAPI, CollectAnalysisExecution collectAnalysisExecution) throws ExceptionWithEvents {
  ...
  private boolean isAlreadyExecutedRule(HumanTaskInstance humanTask, long timeToExecute, Rule rule, PlugTourInput input, ProcessAPI processAPI,CollectAnalysisExecution collectAnalysisExecution) throws ExceptionWithEvents {
  ...
  private void updateCase(HumanTaskInstance humanTask, Rule rule, PlugTourInput input, ProcessAPI processAPI) throws ExceptionWithEvents {
    String variableName = input.getInputStringParameter(cstParamProcessVariableRegistration);
    DataInstance data = null;
    try {
      if (variableName != null && variableName.trim().length() > 0) {
   ...
   * @return
   */
  private List<BEvent> executeRule(HumanTaskInstance humanTask, Rule rule, PlugTourInput input, ProcessAPI processAPI, IdentityAPI identityAPI) {
    List<BEvent> listEvents = new ArrayList<BEvent>();
    String userName = "";
    try {
   ...
   * @return
   */
  private String resolvePlaceHolder(HumanTaskInstance humanTask, User user, Rule rule, PlugTourInput input, String text) {
